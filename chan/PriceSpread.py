...
p1 = np.tile(numUnits.values, [2, 1]).T
p2 = ts.add_constant(-hedgeRatio)[:, [1, 0]]
positions = pd.DataFrame(p1 * p2 * df.values) # results.evec(:, 1)' can be viewed as the capital allocation, while positions is the dollar capital in each ETF.
